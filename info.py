...
CODE = (environ.get('CODE', 'https://graph.org/file/5ba248f8a9914b648d645.jpg'))
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002124030786').split()] #Channel id for auto indexing ( make sure bot is admin )
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002202300892')) #Log channel id ( make sure bot is admin )


auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002110905372') #Channel / Group Id for force sub ( make sure bot is admin )
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1001805509279') # support group id ( make sure bot is admin )
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002244588420') # request channel id ( make sure bot is admin )
...
DEENDAYAL_MOVIE_UPDATE_CHANNEL = int(environ.get('DEENDAYAL_MOVIE_UPDATE_CHANNEL', '-1002142179117')) #Notification of those who verify will be sent to your channel. Enter the ID of the channel you want to send notification to here.
...
DEENDAYAL_VERIFIED_LOG = int(environ.get('DEENDAYAL_VERIFIED_LOG', '-1002202300892')) #Log channel id ( make sure bot is admin )
...
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/+JS2BKjzbYrExNjE9')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/mawhOrzzBackUp2')
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/mawhVerify') # Tutorial video link for opening shortlink website 
IS_TUTORIAL = bool(environ.get('IS_TUTORIAL', False))
MSG_ALRT = environ.get('MSG_ALRT', '~𝑴𝑨𝑾𝑯 𝑶𝒓𝒛𝒛⁠•')
